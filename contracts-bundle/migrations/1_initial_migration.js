...
async function deployWithDeterministicAddress(payingAccount, expectedContractAddress, singleUsedCreatorAddress, gasCosts, rawTx) {
    ...
    // On the RSK node the getCode command returns a null string = '0x00', meanwhile on Ganache returns '0x'
    if (gotCode === '0x00' || gotCode === '0x') {
...
async function deployContract(from, artifact, arguments) {
...
module.exports = (deployer, network, accounts) => {
    deployer.deploy(Migrations).then(() =>
        RNSSuite(web3.currentProvider).then(async (contracts) => {
            ...
            if (network === 'ganache') {
                ...
                await deployEip1820(from);
            }

            // Create and mint ERC20 token
            const erc20 = await deployContract(from, ERC20Mintable);
            console.log(`ERC20 token (used as DOC) deployed to ${erc20.options.address}`);
            await erc20.methods
                .mint(from, web3.utils.toBN('1000000000000000000000').toString())
                .send({
                    from: from
                });
            ...
            let erc777;
            if (network === 'ganache') {
                erc777 = await deployContract(from, ERC777, [
                    'ERC 777',
                    '777',
                    [],
                ]);
            ...
            const erc721 = await deployContract(from, ERC721Mintable);
            console.log(`ERC721 token (NFT we use for RNS) deployed to ${erc721.options.address}`);
            await erc721.methods.mint(from, defaultToken).send({
                from: from
            });

            const bytesLib = await deployContract(from, BytesLib);
            
            console.log(chalk.cyan(`Linking BytesLib into ${ERC721SimplePlacements.contractName}`));
            await ERC721SimplePlacements.link('BytesLib', bytesLib.address);

            // Marketplace registry contract
            const erc721SimplePlacements = await deployContract(from, ERC721SimplePlacements, [contracts.rskOwner.options.address]);

            await erc721SimplePlacements.methods
                .setWhitelistedPaymentToken(contracts.rif.options.address, false, true, false)
                .send({
                    from: from,
                });
